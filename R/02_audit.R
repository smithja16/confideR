...
audit_session <- function(check_rprofile = TRUE, verbose = TRUE) {
  ...
  if (verbose) {
    ...
    if (ide_info$ide_detail$is_rstudio) {
      ...
      cat("    [ ] Addins menu: scan for AI addins, including any NOT on confideR's\n")
      cat("        blocklist - the ones automated detection cannot catch. An addin\n")
      cat("        here means its package is installed; running it from the menu\n")
      cat("        launches its function immediately. Confidential mode blocks\n")
      cat("        blocklisted packages from loading; to remove an addin, uninstall\n")
      cat("        its package with remove.packages().\n")
    } else if (ide_info$ide_detail$is_positron) {
